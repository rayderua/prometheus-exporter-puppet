...
type LastRunSummaryExporter struct {
	...
	version_config     prometheus.Gauge
...
func (self *LastRunSummaryExporter) Collect(ch chan<- prometheus.Metric) {
	...
	self.version_config.Set(float64(last_run_summary.Version.Config))
...
func CreateLastRunSummaryExporter(summary_file string) (*LastRunSummaryExporter, error) {
	return &LastRunSummaryExporter{
		...
		version_config: prometheus.NewGauge(prometheus.GaugeOpts{
			Namespace: "puppet",
			Name:      "version_config",
			Help:      "Puppet Catalog config version ",
		}),
